	  ...
	  for (j = low_left + 1; j <= level; j++)    
	    {
	      ...
	      right_cand = PtimeL[mod(running_right)] + ((float)rand()/(float)RAND_MAX);
	      left_cand = PtimeR[mod(left_consider)] + ((float)rand()/(float)RAND_MAX);
	  ...
	  for (j = left_edge + 1; j <= right_edge; j++) 
	    {
	      ...
	      right_consider ++;
	      left_cand =  PtimeR[mod(left_consider)] + ((float)rand()/(float)RAND_MAX);
	      right_cand = PtimeL[mod(right_consider)] + ((float)rand()/(float)RAND_MAX);
	      DepthR[mod(left_consider)] ++;
	  ...
	  for (j = low_right + 1; j <= level; j++)   
	    {
	      ...
	      right_cand = PtimeL[mod(right_consider)] + ((float)rand()/(float)RAND_MAX);
	      left_cand = PtimeR[mod(running_left)] + ((float)rand()/(float)RAND_MAX);
	      if (left_cand < right_cand)
		{
		  running_value = left_cand;
